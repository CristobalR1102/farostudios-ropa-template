...
import { addProduct, deleteProduct, getConfig, getProducts, resetDemo, saveConfig, updateProduct } from '../lib/demoStore'
...
export default function DemoAdmin() {
  ...
  function handleSaveProduct(e) {
    ...
    if (editingId) {
      setProducts(updateProduct(editingId, payload))
    } else {
      setProducts(addProduct(payload))
    }
    setForm(emptyForm)
    setEditingId(null)
    flash(editingId ? 'Producto actualizado ✓' : 'Producto agregado ✓')
  }
  ...
  function handleReset() {
    if (!confirm('¿Restaurar el catálogo y la configuración de demostración originales?')) return
    resetDemo()
    setProducts(getProducts())
    setConfig(getConfig())
    setForm(emptyForm)
  ...
  return (
    <div className="min-h-screen bg-[#111] font-sans text-[#eee]">
      <div className="bg-[#c9a84c] text-[#0a0a0a] text-center text-[11px] font-bold tracking-widest uppercase py-2 px-4">
        Panel admin de demostración · Los cambios se guardan solo en este navegador ·{' '}
        <a href="/demo" className="underline underline-offset-2">Ver tienda</a>
      </div>

      <div className="bg-[#0a0a0a] px-6 py-4 flex items-center justify-between border-b border-[#333]">
        <span className="text-[#c9a84c] tracking-widest uppercase text-sm font-bold">Admin · Demo</span>
        <button onClick={handleReset} className="text-[11px] text-[#777] hover:text-[#eee] tracking-wide uppercase border border-[#333] px-3 py-1.5 rounded cursor-pointer bg-transparent transition-colors">
          Restaurar demo
        </button>
      </div>
          ...
          <div>
            ...
            <form onSubmit={handleSaveProduct} className="bg-[#1e1e1e] border border-[#333] rounded-lg p-5 mb-8 grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
              ...
              <div className="col-span-full flex gap-3">
                <button type="submit" className="bg-[#c9a84c] text-[#0a0a0a] font-bold py-2 px-6 rounded text-xs tracking-widest uppercase cursor-pointer border-none">
